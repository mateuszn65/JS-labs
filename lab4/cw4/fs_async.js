...
export function check(name, response){
    ...
    fs.readdir(path, (err, files)=>{
        ...
        files.forEach((file, index, array) =>{
            ...
            fs.stat(new_path, (err, stats)=>{
                ...
                if (err)
                    throw err
                if (stats.isDirectory() && file == name){
                    found = true
                    ...
                    response.end()
                    return
                }else if (stats.isFile() && file == name){
                    found = true
                ...
                }else if (path == '.' && index == array.length - 1 && !found){
                    response.write(`${msg}`)
                    console.log("Sending the response")
                    response.end()
                    return 
...
/**
	...
	*/
    function requestListener(request, response) {
        ...
        if (url.pathname == '/submit') { // Processing the form content, if the relative URL is '/submit'
            ...
            if (request.method == 'GET'){// If the GET method was used to send data to the server
                ...
                //response.write(`${msg}`); // "url.searchParams.get('name')" contains the contents of the field (form) named 'name'
            } 
            ...
            response.write(`<form method="GET" action="/submit">
                                <label for="name">Give your name to search</label>
                                <input name="name">
                                <br>
                                <input type="submit">
                                <input type="reset">
    ...
    var server = createServer(requestListener); // The 'requestListener' function is defined above
    server.listen(8080);
